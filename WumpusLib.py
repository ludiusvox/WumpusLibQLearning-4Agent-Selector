...
class WumpusGame(object):
    ...
    def __init__(self, PrintMessages=True):
        ...
        self.env = self._make_standard_cave()
        # Now populate the cave with elements including
        # the and the gold itself.
        self.threats = {}
        self.pits = set([])
        self.gold_pos = -1
        self.exit_pos = -1
    ...
    def get_discrete_state(self,state):
        self.discrete_state = (state - self.env.observation_space.low)/self.discrete_os_win_size
        return tuple(self.discrete_state.astype(np.int))  # we use this tuple to look up the 3 Q values for the available actions in the q-table
    ...
    def _populate_cave(self):
        ...
        for threat in ['pit', 'pit', 'pit', 'pit']:
            ...
            outp = open("myOutFile3.txt", "a")
            outp.writelines(str(pos)+" ")
            self.pits.add(pos)
        ...
        oute = open("myOutFile2.txt", "w")
        oute.writelines(str(pos))
        self.exit_pos = pos
    ...
    def getcount(self):
        ...
        if (self.countset == 50):
            print("Out of ability movement points, Score is 0")
            sys.exit()
    def get_percepts(self):
        ...
        for NeighborPos in NeighboringPositions:
            if NeighborPos < 0:
                PerceptSet.append("boundary")
            
            # Check if the wumpus is here and add stench.
            elif (NeighborPos == self.wumpus_pos):
                PerceptSet.append("stench")
            
            # Check if the pit is here and add breeze.
            elif (NeighborPos in self.pits):
                PerceptSet.append("breeze")
            
            # Try to find a safe position and not pitfall
            elif (NeighborPos in SafePos):
                PerceptSet.append("safe")
    ...
    def _goto_room(self, RoomNum):
        ...
        if ((RoomNum < 1) or (RoomNum > 25)):
            ...
            self.player_pos = -1
            self.LifePoints = self.LifePoints - 1000
            print("score is: 0")
            sys.exit()
    ...
    def get_curr_room(self):
        ...
        return("current room is: " + str(self.player_pos))
    ...
    def exit(self):
        ...
        if (self.player_pos == self.exit_pos):
            if (self.has_gold == True):
                ...
                self.msg("Score is" + str(1000 + 1000 - self.countset*20))
                sys.exit()
                return(True)
            else:
                self.msg("Exiting game without gold!")
                self.msg("Score is " + str(1000 - self.countset*20))
                sys.exit()
                return(False)
...
class SimpleReflex(object):
    ...
    def agent_func(self, Percepts):
        ...
        """
        print("Perception is north: " + str(Percepts[0])  ) 
        print("Perception is west: " + str(Percepts[-1]))
        print("Perception is east: " +str(Percepts[-2]))
        print(("Perception is south: " + str(Percepts[-3])))
    ...
    def play_game(self):
        ...
        for I in range(100):
            ...
            count = self.Game.getcount()
            finalcount = ("terminated at " + str(self.Game.countset))
            print(finalcount)
            ...
            self.Game.action_wrapper(NewAction)
class SimpleModel(object):
    
    
    """
    Define a more advanced reflex that attempts to solve more scenarios
    """
    ...
    def agent_func(self, Percepts):
        ...
        if ("breeze" in Percepts[0]):

            return("GoSouth")
        elif ("stench" in Percepts[0]):
            return("ShootNorth")
        elif ("stench" in Percepts[-1]):
            return("ShootWest")
        elif ("stench" in Percepts[-2]):
            return("ShootEast")
        elif ("stench" in Percepts[-3]):
            return("ShootSouth")
        elif ("Boundary" in Percepts[0]):
            return("GoEast")
        elif ("Boundary" in Percepts[-1]):
            return("GoEast")
        elif ("Boundary" in Percepts[-2]):
            return("GoSouth")
        elif ("Boundary" in Percepts[-3]):
            return("GoNorth")
        # If we see the gold pick it up.
        elif ("glint" in Percepts):
            return("PickupGold")
